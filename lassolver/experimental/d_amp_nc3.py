...
class D_AMP_nc3(D_Base):
    def __init__(self, A, x, noise, P):
        super().__init__(A, x, noise, P)
        self.amps = [damp_nc3(self.A_p[p], x, self.noise[p], self.M) for p in range(self.P)]
    ...
    def _update_s(self, w_p, beta):
        w = np.sum(w_p, axis=0)
        s = soft_threshold(w, beta)
        self.s = s
